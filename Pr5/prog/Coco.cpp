...
void auto_repeat(const Position& pos){
  for(int i = 0; i < 30; i++){
    double RA = 15.0 * Rad * Ddd(0, i, 0);
    double dec = Rad * Ddd(0, 0, 0);
    Position p = pos;
    
    p.m_R = Vec3D(Polar(RA, dec, 1.0));
...
void task(){
   double mjd = Mjd(2000, 8, 15, 21, 0, 0.0);
   double jd = mjd + 2400000.5;
   std::cout << "JD: " << jd << "\n";
   std::cout << "Modify JD: " << mjd << "\n";
}
